...
libs <- c("tools", "tidyverse", "icesTAF", "stringr", "flextable", "ggplot2", "viridis", "purrr", "quantreg", "RColorBrewer", "readxl") 
...
load("data/issues/data_call_base/all_odd_base.RData")
...
After the first evaluation of individual biometric data in the WGEEL database several issues were discovered, such as incomplete reporting or potentially incorrect data. In 2025 a data call was sent to data providers asking for correction, validation or provision of missing data. Further, data providers were asked to provide any individual biometric data that was not reported before, stressing it's relevance for the DIASPARA project and the envisioned assessment approach.
This script/report re-evaluates the data after corrections, validations and integration of new data. Ultimately, based on these findings the data suitable for the LHT models was selected.


## 1. Data call response and 2025 call summary

This section summarizes the response to the data call on issues identified in the individual biometric data, as well as new data entries during the 2025 ICE WGEEL data call.

### Tab 1.1: Summary of the number of individuals in the database at the start of the project, identified issues/corrections and the total after the 2025 ICES data call. Note, that the difference in the total number of individuals does not necessarily reflect the number of new integrations since existing individual biometric data may have been deleted from the database.

```{r data call response summary, echo=FALSE, warning = FALSE, message = FALSE, fig.align = "left", ft.align="left"}

#COUNT RESPONSES BY COUNTRY

# Define the path from project root
folder_path <- "../call_responses_base"
...
combined_df <- bind_rows(mget(df_list)) %>% 
  mutate(action = na_if(action, "NA"),
         action = na_if(action, ""),
         mei_fi_id = as.integer(mei_fi_id)) 
...
#nrow(conflicts) #checked if there is conflicts with different actions for the same ID before removing duplicates. None there.
...
#create dataframe with only those corrected or validated
df_corrected <- combined_df_clean %>% 
  filter(!is.na(action)) 

#create summary of answers by country
summary_individuals_corrected <- df_corrected %>% 
count(ser_cou_code, name = "validated_corrected")

#count all individuals per country
summary_individual_all_base <- individual_all_base %>% 
  count(ser_cou_code, name = "total_before_call")

#count all individuals with issues per country
summary_all_odd_base_country <- all_odd_base %>%
  count(ser_cou_code, name = "issues")

#create dataframe with total after 2025 call
summary_all_new_country <- individual_all %>% 
  count(ser_cou_code, name = "total_after_call")

#create summary of added fish after 2025
summary_added_fish <- left_join(summary_individual_all_base, summary_all_new_country, by = "ser_cou_code") %>% 
  select(-total_before_call)

#knit to a single df
summary_issues_base <- list(summary_individual_all_base, summary_all_odd_base_country, summary_individuals_corrected, summary_added_fish) %>%
  reduce(left_join, by = "ser_cou_code") %>% 
  bind_rows(summarise(., ser_cou_code = "Sum", across(where(is.numeric), \(x) sum(x, na.rm = TRUE))))

summary_issues_base %>%
  flextable() %>%
  set_header_labels(
    ser_cou_code = "Country",
    total_before_call = "Ind. before 2025 call",
    issues = "Ind. with potential issues",
    validated_corrected = "Ind. corrected/validated",
    total_after_call = "Ind. after 2025 call"
  ) %>% 
  colformat_double(big.mark = " ", digits = 0) %>% 
  colformat_int(big.mark = " ") %>%
  hline(i = nrow(summary_issues_base) - 1, part = "body", border = officer::fp_border(width = 1)) %>%
  autofit()
...
In response to the data call adressing potential issues in the biometric data, 7 out of 16 countries replied, providing validation or corrections for about 24338 individual or 30% of the identified issues. Following the 2025 ICES data call 91053 fish were added to the database. 


```{r data call issues summary, echo=FALSE, warning = FALSE, message = FALSE, fig.align = "left", ft.align="left"}
  
#count issues by country
summary_all_odd_base_issue <- all_odd_base %>%
  separate_longer_delim(issue, delim = ",") %>%
  mutate(issue = trimws(issue)) %>%
  count(ser_cou_code, issue) %>% 
  arrange(ser_cou_code, desc(n)) %>% 
  mutate(issue = reorder(issue, -n))

summary_all_odd_after_correction <- all_odd_base %>% 
...
# 1. Grab the 12 colors from the Paired palette
my_colors <- brewer.pal(12, "Paired")

# 2. Add a 13th color that is definitely NOT white
my_colors <- c(my_colors, "black")

#create a plot with issues by country before correction
plot_issues_before <- ggplot(data = summary_all_odd_base_issue, aes(x = ser_cou_code, y = n, fill = issue)) +
  geom_col(position = "dodge", alpha = 0.7, color = "white") +
  scale_fill_manual(values = my_colors) +
  labs(
    title = "Issues per Country before validation",
    x = "Country Code",
    y = "Number of Occurrences"
  ) +
  theme_bw()

#create a plot with issues by country after correction
plot_issues_after <- ggplot(data = summary_all_odd_after_correction, aes(x = ser_cou_code, y = n, fill = issue)) +
  geom_col(position = "dodge", alpha = 0.7, color = "white") +
  scale_fill_manual(values = my_colors) +
  labs(
    title = "Issues per Country after validation (excluding new data)",
    x = "Country Code",
    y = "Number of Occurrences"
  ) +
  theme_bw()
...
#### Fig 1.1: Summary of issues identified by country and type of the issue



## 2. Extract & remove fish without length, independent of stage
...
#### Tab 2.1: Eels removed due to missing length; by stage and sex.
...
## 3. Find eels where there is an issue with lifestage/sex 
...
#### Tab 3.1 Number of eels where there is an issue with lfs_code, sex, or both, by country and life stage (see header).
...
df_all_lfs_sex_odd <- df_all_length %>% 
  ...
  mutate(valid = mei_fi_id %in% df_corrected$mei_fi_id) %>% 
  mutate(issue = ifelse(valid == TRUE, "validated", issue))
...
create_summary_lfs <- function(lfs) {
  ...
  filtered_lfs %>%
    ...
    summarize(lfs_dont_match = sum(lfs_dont_match, na.rm = TRUE),
              ...
              validated = sum(valid, na.rm = TRUE),
              Remaining = sum(!valid, na.rm = TRUE),
...
#### Fig 3.1 Number of eels where there is an issue with lfs_code, sex, or both by country and life stage (as provided by countries)


## 4. Extract issues with sex
...
#### Tab 4.1: Numbers of eels with an issue in sex determination, by country and life stage (see header).
...
#### Fig 4.1: Number of fish with obvious an issue in sex by life stage and country 




## 5. Eels without precise location
...
create_summary_coordinates <- function(lfs) {
  ...
  summarize(total_in_emus = n(),
            ...
            validated = sum(valid, na.rm = TRUE),
            Remaining = sum(!valid, na.rm = TRUE) - no_issue,
            .groups = "drop")
}
...
## 6. Extract eels with obvious issues in length, weight or age
...
#### Tab 6.1 (top): Number of fish with obvious errors by stage and issue type 

#### Tab 6.2 (bottom): Number of individuals remaining in the dataset by lifestage
...
# remove odd eels from data
df_all_clean <- anti_join(df_all_length, df_all_obvious_odd %>% filter(valid == FALSE), by = "mei_fi_id")
...
## 7. Extract issues with weight or length in glass eel data (lfs_code = G)
...
df_G_clean <- anti_join(df_G, df_G_obvious_odd %>% filter(valid == FALSE), by = "mei_fi_id")
...
df_G_wl_odd <- df_G_clean %>% 
  ...
  mutate(issue = ifelse(valid == TRUE, "validated", "weight_length_off")) %>% 
...
df_G_plot <-  df_G_clean %>% 
  mutate(issue = case_when(valid == TRUE ~ "validated",
                           weight_length_off == TRUE ~ "weight_length_off",
                           TRUE ~ "no_issue"))
...
plot_G_wl_log <- ggplot(data = df_G_plot, aes(x = log(lengthmm), y = log(weightg))) +
  ...
  scale_color_manual(values = c("no_issue" = "#00204D", "weight_length_off" = "#FF9F00", "validated" = "#C71585")) + 
...
## 8. Extract issues in yellow eel data (lfs_code = Y)

### 8.1 Yellow eels where weight and length do not match
...
#### Tab 8.1: Number of yellow eels with an issue in weight or length by country
...
#...and create a clean df without them
df_Y_clean <- anti_join(df_Y, df_Y_obvious_odd %>% filter(valid == FALSE), by = "mei_fi_id")
...
#### Fig 8.1: Length-weight relationship for yellow eels on log/log and natural scale. Black line shows the quantile regression model (tau = 0.5), fit on the log/log-scale. Grey ribbon indicates 99.9% conditional prediction intervals which was used to identify data with no issue (blue dots) vs data with potential issues (yellow dots) which was extracted for validation/clarification. 
...
### 8.2 Highlight yellow eels with odd weight_length patterns
...
```
#### Fig 8.2: Panel of EMUs in France highlighting some data with ominous patterns. 

While the only country responding to this specific isue was FR, it was confirmed these lines result from averaging weight (batch weighing) while length are measured individually, hence this is considered a non-issue for the modeling of LHTs (weight not used).


## 9. Extract issues in silver eel data (lfs_code = S)
...
#...and create a clean df without them
df_S_clean <- anti_join(df_S, df_S_obvious_odd %>% filter(valid == FALSE), by = "mei_fi_id")
...
#create summarS table of odd data
...
#### Fig 9.1: Length-weight relationship for silver eels on log/log and natural scale. Black line shows the quantile regression model (tau = 0.5), fit on the log/log-scale. Grey ribbon indicates 99.9% conditional prediction intervals which was used to identify data with no issue (blue dots) vs data with potential issues (yellow dots) which was extracted for validation/clarification. 
...
## 10. Summarize odd data and remove those relevant for LHT model from the dataset

```{r create data for LHT model, echo=FALSE, warning = FALSE, message = FALSE, fig.align = "left", ft.align="left"}

#compile all odd data
files <- list.files("./data/issues/odd_data", 
                    pattern = "_odd\\.RData$", 
                    full.names = TRUE)

#extract names from "files" (dfs already in environment from the script above)
obj_names <- file_path_sans_ext(basename(files))

#combine them into one dataframe and drop the column issue
all_odd <- mget(obj_names) %>% 
  bind_rows() %>% 
  select(-issue) %>%
  group_by(mei_fi_id) %>%
...
all_odd <- all_odd %>% 
  ...
  select(mei_fi_id, fi_id_cou, ser_cou_code, lfs_code, lengthmm, ageyear, issue)


#create a df with all odd individuals that were validated
all_odd_validated <- all_odd %>%
  filter(str_detect(issue, "valid"))

#create a df with all odd individuals that were not validated
all_odd_unvalidated <- all_odd %>% anti_join(all_odd_validated, by = "mei_fi_id")
  
# create a vector with issues considered highly relevant to the analyses
relevant <- c("weight_length_off|ser_y_out_of_range|too_short|unrealistic_length|unrealistic_weight|too_long|too_short|too_old|too_light")

#create a df with all odd individuals that have relevant issues
all_odd_relevant <- all_odd %>%
  filter(str_detect(issue, relevant))

#create a dataframe with all individuals that have relevant issues and are not validated
all_odd_relevant_unvalidated <- all_odd_unvalidated %>%
  filter(str_detect(issue, relevant))

#create a dataframe with all individuals that have minor issues and are not validated
all_odd_minor_unvalidated <- all_odd_unvalidated %>%
  filter(!str_detect(issue, relevant))

#create a dataframe with all eels except those with any unvalidated issues 
forLHT_safe <- df_all_length %>% 
  anti_join(all_odd_unvalidated, by = "mei_fi_id")

#create a dataframe with all eels except those with highly relevant unvalidated issues 
forLHT_middleground <- df_all_length %>% 
  anti_join(all_odd_relevant_unvalidated, by = "mei_fi_id")

#store relevant tables
save(df_all_length, file = "data/issues/all_ind_with_length.RData")
save(forLHT_safe, forLHT_middleground, file = "data/issues/data_for_LHTs.RData")
save(all_odd_base, df_corrected, all_odd, all_odd_validated, all_odd_unvalidated, all_odd_relevant_unvalidated, all_odd_minor_unvalidated, relevant, file = "data/issues/odd_data.RData")
