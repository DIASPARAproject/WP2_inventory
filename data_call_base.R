...
logi_cols <- names(Filter(is.logical, all_odd_base))

#add issues column  
all_odd_base <- all_odd_base %>% 
...
split_dfs <- split(all_odd_base, all_odd_base$ser_cou_code)

# Save each dataframe to a CSV comma separated
invisible(lapply(names(split_dfs), function(code) {
  write.table(
    split_dfs[[code]],
    file = file.path("data/issues/data_call_base/comma", paste0("validate_", code, ".csv")),
    sep = ",",
    row.names = FALSE,
    col.names = TRUE,
    quote = TRUE
...
invisible(lapply(names(split_dfs), function(code) {
  write.table(
    split_dfs[[code]],
    file = file.path("data/issues/data_call_base/semicolon", paste0("validate_", code, ".csv")),
    sep = ";",
    row.names = FALSE,
    col.names = TRUE,
    quote = TRUE
...
# store summary of odd data
save(all_odd_base, file = "data/issues/data_call_base/all_odd_base.RData")