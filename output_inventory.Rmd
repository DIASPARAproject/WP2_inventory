...
libs <- c("tidyverse", "icesTAF", "stringr", "flextable", "sf", "ggplot2", "viridis", "RColorBrewer") 
...
load("data/issues/all_ind_with_length.RData")
load("data/issues/data_for_LHTs.RData")
load("data/issues/odd_data.RData")
...
## 1.1 Counts of eels, countries, EMUs, sites

```{r numbers for text, echo=FALSE, warning = FALSE, message = FALSE}

# 2.1 Totals ####
#total number of individuals
total_individuals_n <- nrow(individual_all)
cat(paste("Total number of eels:", total_individuals_n, sep = " "), "\n")
cat("\n")

#total number of individuals base
total_individuals_n_base <- nrow(individual_all_base)
cat(paste("Total number of eels base:", total_individuals_n_base, sep = " "), "\n")
cat("\n")

#total number of individuals base with issues
total_individuals_n_base_issues <- nrow(all_odd_base)
cat(paste("Total number of eels base identified with issues:", total_individuals_n_base_issues, sep = " "), "\n")
cat("\n")

#total number of individuals base with issues answered
total_individuals_n_base_issues_answered <- nrow(df_corrected)
cat(paste("Total number of eels base identified with issues answered:", total_individuals_n_base_issues_answered, sep = " "), "\n")
cat("\n")

#total number of glass eels in the dataset
total_individuals_glass <- nrow(individual_all %>% filter(lfs_code == "G"))
cat(paste("Total number of glass eels:", total_individuals_glass, sep = " "), "\n")
cat("\n")

#total number with length
length_individuals_length <- nrow(df_all_length)
cat(paste("Total number of eels with length:", length_individuals_length, sep = " "), "\n")
cat("\n")

#total number validated
length_validated_n <- nrow(all_odd_validated)
cat(paste("Total number of eels with validated/corrected issues in db:", length_validated_n, sep = " "), "\n")
cat("\n")

#total number with issues
length_unvalidated_n <- nrow(all_odd_unvalidated)
cat(paste("Total number of eels with remaining issues in db:", length_unvalidated_n, sep = " "), "\n")
cat("\n")

#total number with minor issues
length_unvalidated_minor_n <- nrow(all_odd_minor_unvalidated)
cat(paste("Total number of eels with minor issues:", length_unvalidated_minor_n, sep = " "), "\n")
cat("\n")

#total number of countries
total_countries <- unique(individual_all$ser_cou_code)
total_countries_n <- length(unique(individual_all$ser_cou_code))
cat(sprintf("Total number of countries: %d (%s)\n", total_countries_n, sprintf("%s", paste(total_countries, collapse = ", "))))
cat("\n")

#total number of EMUs
total_emus <- unique(individual_all$ser_emu_nameshort)
total_emus_n <- length(unique(individual_all$ser_emu_nameshort))
...
series_individuals_n <- nrow(individual_all %>% filter(source == "series"))
cat(paste("Total number of eels in series:", series_individuals_n, sep = " "), "\n")
cat("\n")

#total number of countries providing time series data
series_countries <- unique((individual_all %>% filter(source == "series"))$ser_cou_code)
series_countries_n <- length(unique((individual_all %>% filter(source == "series"))$ser_cou_code))
cat(sprintf("Total number of countries in series: %d (%s)\n", series_countries_n, sprintf("%s", paste(series_countries, collapse = ", "))))
cat("\n")

#total number of series
series_sites <- unique((individual_all %>% filter(source == "series"))$ser_nameshort)
series_sites_n <- length(unique((individual_all %>% filter(source == "series"))$ser_nameshort))
...
sampling_individuals_n <- nrow(individual_all %>% filter(source == "sampling"))
...
sampling_countries <- unique((individual_all %>% filter(source == "sampling"))$ser_cou_code)
sampling_countries_n <- length(unique((individual_all %>% filter(source == "sampling"))$ser_cou_code))
...
sampling_emus <- unique((individual_all %>% filter(source == "sampling"))$ser_emu_nameshort)
sampling_emus_n <- length(unique((individual_all %>% filter(source == "sampling"))$ser_emu_nameshort))
...
# DCF Data
DCF_only <- individual_all %>% filter(str_detect(sai_samplingobjective, "DCF"))
...
# EMP data
EMP_only <- individual_all %>% filter(str_detect(sai_samplingobjective, "EMP"))
...
cat("Total number eels reported with length less 150mm in OSD: ", sampling_length_150minus_n, sep = "", "\n")
cat("\n")

```
## 1.2 Male/Female Proportions
...
### 2.3 by year
\n
```{r inventory by year, echo=FALSE, warning = FALSE, message = FALSE}

# Create the summary table
country_year <- individual_all %>%
  group_by(fi_year, ser_cou_code) %>%
  summarize(row_count = n(), .groups = "drop")

#order categories Y
ordered_categories_Y <- c(
  "invalid",
  "other",
  "other_stage",
  "stage_plus_missing",
...
#code availabilty per year (growth)
availability_year_growth <- individual_all %>%
  left_join(all_odd_unvalidated %>% select(mei_fi_id, issue), by = "mei_fi_id") %>% 
  mutate(availability = case_when(str_detect(issue, relevant) & !is.na(issue) ~ "invalid",
                    !is.na(ageyear) & !is.na(lengthmm) & lfs_code == "Y" & !is.na(sex) ~ "suitable_Y",
                    is.na(ageyear) & !is.na(lengthmm) & lfs_code == "Y" & !is.na(sex) ~ "no_age_Y",
                    !is.na(ageyear) & is.na(lengthmm) & lfs_code == "Y" & !is.na(sex) ~ "no_length_Y",
                    !is.na(ageyear) & !is.na(lengthmm) & lfs_code == "Y" & is.na(sex) ~ "no_sex_Y",
                    is.na(ageyear) & !is.na(lengthmm) & lfs_code == "Y" & is.na(sex) ~ "no_age_sex_Y",
                    !is.na(ageyear) & is.na(lengthmm) & lfs_code == "Y" & is.na(sex) ~ "no_length_sex_Y",
                    is.na(ageyear) & is.na(lengthmm) & lfs_code == "Y" & !is.na(sex) ~ "no_age_length_Y",
                    lfs_code == "Y" ~ "all_missing_Y",
                    !is.na(ageyear) & !is.na(lengthmm) & (is.na(lfs_code) | lfs_code == "GY" | lfs_code == "YS") & !is.na(sex) ~ "no_stage",
                    lfs_code == "GY" | lfs_code == "YS" | is.na(lfs_code) ~ "stage_plus_missing",
                    lfs_code == "G" | lfs_code == "S" ~ "other_stage",
                    TRUE ~ "other")) %>% 
  #filter(availability == "all_missing_Y")
  mutate(availability = factor(availability, levels = ordered_categories_Y)) %>%
  group_by(fi_year, availability) %>%
  summarize(row_count = n(), .groups = "drop")

#order categories S
ordered_categories_S <- c(
  "invalid",
  "other",
  "other_stage",
  "stage_plus_missing",
...
#code availabilty per year (maturation)
availability_year_mat <- individual_all %>%
  left_join(all_odd_unvalidated %>% select(mei_fi_id, issue), by = "mei_fi_id") %>% 
  mutate(availability = case_when(str_detect(issue, relevant) & !is.na(issue) ~ "invalid",
                    !is.na(lengthmm) & lfs_code == "S" & !is.na(sex) ~ "suitable_S",
                    is.na(lengthmm) & lfs_code == "S" & !is.na(sex) ~ "no_length_S",
                    !is.na(lengthmm) & lfs_code == "S" & is.na(sex) ~ "no_sex_S",
                    is.na(lengthmm) & lfs_code == "S" & is.na(sex) ~ "no_length_sex_S",
                    lfs_code == "S" ~ "all_missing_S",
                    !is.na(lengthmm) & (is.na(lfs_code) | lfs_code == "YS") & !is.na(sex) ~ "no_stage",
                    lfs_code == "G" | lfs_code == "S"| lfs_code == "GY"| lfs_code == "Y" ~ "other_stage",
                    lfs_code == "YS" | is.na(lfs_code) ~ "stage_plus_missing",
                    TRUE ~ "other")) %>% 
  #filter(availability == "other")
  mutate(availability = factor(availability, levels = ordered_categories_S)) %>%
  group_by(fi_year, availability) %>%
  summarize(row_count = n(), .groups = "drop")

#  Grab the 12 colors from the Paired palette
my_colors <- brewer.pal(12, "Paired")



# Generate the stacked bar charts
inventory_by_country <- ggplot(country_year, aes(x = fi_year, y = row_count, fill = ser_cou_code)) +
...
inventory_by_availability_Y <- ggplot(availability_year_growth, aes(x = fi_year, y = row_count, fill = availability)) +
  geom_bar(stat = "identity", position = "stack") +
...
# Generate the stacked bar chart
inventory_by_availability_S <- ggplot(availability_year_mat, aes(x = fi_year, y = row_count, fill = availability)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = my_colors) +
  labs(
    x = "Year",
...
inventory_by_availability_Y
inventory_by_availability_S

suit_growth <- sum(availability_year_growth$row_count[which(availability_year_growth$availability == "suitable_Y")])
suit_mat <- sum(availability_year_mat$row_count[which(availability_year_mat$availability == "suitable_S")])
totY <- nrow(individual_all %>% filter(lfs_code == "Y"))
totS <- nrow(individual_all %>% filter(lfs_code == "S"))

cat(sprintf("Eels suitable for growth analyses", print(suit_growth), sep = " "), "\n")
cat(sprintf("Eels suitable for mat analyses", print(suit_mat), sep = " "), "\n")
cat(sprintf("Eels Y", print(totY), sep = " "), "\n")
cat(sprintf("Eels S", print(totS), sep = " "), "\n")
