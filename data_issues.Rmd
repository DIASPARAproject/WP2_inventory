...
## 5. Extract eels with obvious issues in length or weight
...
df_G_clean <- df_G_clean %>% 
  mutate(prediction = predict(model_G_wl, newdata = df_G_clean),
         residual = log(weightg) - prediction) %>%  # Use signed residuals
  mutate(prediction_lower = prediction + residual_thresholds_G[1],  # Lower bound
         prediction_upper = prediction + residual_thresholds_G[2],  # Upper bound
         weight_length_off = residual < residual_thresholds_G[1] | residual > residual_thresholds_G[2])

#extract data where weight length relationship is below 0.1 or above 0.9 quantile
...
#create a dataframe excluding too long/too heavy for plotting
...
plot_G_wl_log <- ggplot(data = df_G_plot, aes(x = log(lengthmm), y = log(weightg))) +
  geom_point(aes(color = issue), shape = 16, alpha = 0.4, size = 1) +
  scale_color_viridis_d(option = "cividis") +  # Use viridis discrete color scale
...
df_Y_clean <- df_Y_clean %>% 
  mutate(prediction = predict(model_Y_wl, newdata = df_Y_clean),
         residual = log(weightg) - prediction) %>%
  mutate(prediction_lower = prediction + residual_thresholds_Y[1],  # Lower bound
         prediction_upper = prediction + residual_thresholds_Y[2],  # Upper bound
         weight_length_off = residual < residual_thresholds_Y[1] | residual > residual_thresholds_Y[2])
...
plot_Y_wl <- ggplot(data = df_Y_plot, aes(x = lengthmm, y = weightg)) +
  ...
  geom_line(aes(x = lengthmm, y = exp(prediction)), color = "black") +
  geom_ribbon(aes(ymin = exp(prediction_lower), ymax = exp(prediction_upper)), fill = "gray70", alpha = 0.3) +
  scale_x_continuous(limits = c(min(df_Y_plot$lengthmm), max(df_Y_plot$lengthmm[which(!is.na(df_Y_plot$weightg))]))) +
  theme_bw()
...
Note, fish with length < 65mm, or weight < 1g are not shown and were not used to fit the weigh-length relationship, but they are included in the extracted glass eels with odd biometric data for validation/clarification.
...
df_S_clean <- df_S_clean %>% 
  mutate(prediction = predict(model_S_wl, newdata = df_S_clean),
         residual = log(weightg) - prediction) %>%
  mutate(prediction_lower = prediction + residual_thresholds_S[1],  # Lower bound
         prediction_upper = prediction + residual_thresholds_S[2],  # Upper bound
         weight_length_off = residual < residual_thresholds_S[1] | residual > residual_thresholds_S[2])
...
Note, fish with length < 150mm, or weight < 10g are not shown and were not used to fit the weigh-length relationship, but they are included in the extracted glass eels with odd biometric data for validation/clarification.
