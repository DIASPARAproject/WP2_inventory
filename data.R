...
#create a dataframe with all individuals where length is less than 100mm
individual_removed <- individual_all %>% 
  filter(lengthmm < 30)

#create dataframe where fish with length less than 100mm are removed
individual_cleaned <- anti_join(individual_all, individual_removed, by = "mei_fi_id")

#create dataframe where fish with length less than 100mm are removed for the most recent 10 years (for spatial analyses)
