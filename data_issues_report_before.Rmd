...
#load data 
load("data/individual_all_base.RData")
...
df_no_length_base <- individual_all_base %>% filter(is.na(lengthmm)) %>%
  mutate(issue = "no_length")

# remove odd eels from data
df_all_length_base <- anti_join(individual_all_base, df_no_length_base, by = "mei_fi_id") 

# create summary table of removals
summary_no_length_base <- df_no_length_base %>% 
            group_by(lfs_code) %>% 
...
flextable(summary_no_length_base%>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE))))

#store summary table as csv
write.table(summary_no_length_base, file = "data/issues/tables/summary_no_length_base.csv", row.names = FALSE, sep = ";")

# store odd data
save(df_no_length_base, file = "data/issues/removals/df_no_length_base.RData")
...
df_all_lfs_sex_odd_base <- df_all_length_base %>% 
...
create_summary_lfs <- function(lfs) {
  if (missing(lfs)) {
  filtered_lfs <- df_all_lfs_sex_odd_base   
  }
  else if(is.na(lfs)) {
  filtered_lfs <- df_all_lfs_sex_odd_base %>% filter(is.na(lfs_code))  
  } else {
  filtered_lfs <-  df_all_lfs_sex_odd_base %>% filter(lfs_code == lfs) 
  }
  filtered_lfs %>%
...
summary_all_lfs_sex_odd_base  <- create_summary_lfs()
summary_Y_lfs_sex_odd_base  <- create_summary_lfs("Y")
summary_S_lfs_sex_odd_base  <- create_summary_lfs("S")
summary_YS_lfs_sex_odd_base <- create_summary_lfs("YS")
summary_NA_lfs_sex_odd_base <- create_summary_lfs("NA")

#print tables
flextable(summary_all_lfs_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("TOTAL")

flextable(summary_Y_lfs_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = Y")

flextable(summary_S_lfs_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = S")

flextable(summary_YS_lfs_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = YS")

flextable(summary_NA_lfs_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = NA")

#plot yellow eels where there is an issue with sex
plot_all_lfs_sex_odd_base <- ggplot(df_all_lfs_sex_odd_base) +
...
#print plot
plot_all_lfs_sex_odd_base

#save csv of summaries                    
write.table(summary_Y_lfs_sex_odd_base, file = "data/issues/tables/summary_Y_lfs_sex_odd_base.csv", row.names = FALSE, sep = ";")
write.table(summary_S_lfs_sex_odd_base, file = "data/issues/tables/summary_S_lfs_sex_odd_base.csv", row.names = FALSE, sep = ";")
write.table(summary_YS_lfs_sex_odd_base, file = "data/issues/tables/summary_all_YS_sex_odd_base.csv", row.names = FALSE, sep = ";")

# save plots
ggsave("plot_all_lfs_sex_odd_base.png", plot_all_lfs_sex_odd_base, path = "data/issues/plots/", device = "png", dpi = 300, width = 16, height = 10, units = "in")

# store odd data
save(df_all_lfs_sex_odd_base, file = "data/issues/odd_data/df_all_lfs_sex_odd_base.RData")
...
df_all_sex_odd_base <- df_all_length_base %>% 
...
create_summary_sex <- function(lfs) {
  if(missing(lfs)) {
  filtered_sex <- df_all_sex_odd_base  
  }
  else if(is.na(lfs)) {
  filtered_sex <- df_all_sex_odd_base %>% filter(is.na(lfs_code))  
  } else {
  filtered_sex <-  df_all_sex_odd_base %>% filter(lfs_code == lfs) 
  }
  filtered_sex %>% 
  group_by(ser_cou_code) %>% 
...
#apply function for each life stage
summary_all_sex_odd_base <- create_summary_sex()
summary_Y_sex_odd_base <- create_summary_sex("Y")
summary_S_sex_odd_base <- create_summary_sex("S")
summary_YS_sex_odd_base <- create_summary_sex("YS")
summary_NA_sex_odd_base <- create_summary_sex(NA)

# print report tables
flextable(summary_all_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("TOTAL")

flextable(summary_Y_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = Y")

flextable(summary_S_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = S")

flextable(summary_YS_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = YS")

flextable(summary_NA_sex_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = NA")

#plot yellow eels where there is an issue with sex
plot_all_sex_odd_base <- ggplot(df_all_sex_odd_base) +
  geom_histogram(aes(x = ser_cou_code, fill = issue), 
                 stat = "count", position = "stack", color = "black", alpha = 0.8) +
...
#print plot
plot_all_sex_odd_base

#store tables as csv
write.table(summary_Y_sex_odd_base, file = "data/issues/tables/summary_Y_sex_odd_base.csv", row.names = FALSE, sep = ";")
write.table(summary_S_sex_odd_base, file = "data/issues/tables/summary_S_sex_odd_base.csv", row.names = FALSE, sep = ";")
write.table(summary_YS_sex_odd_base, file = "data/issues/tables/summary_YS_sex_odd_base.csv", row.names = FALSE, sep = ";")
write.table(summary_NA_sex_odd_base, file = "data/issues/tables/summary_NA_sex_odd_base.csv", row.names = FALSE, sep = ";")

#store plots
ggsave("plot_all_sex_odd_base.png", plot_all_sex_odd_base, path = "data/issues/plots/", device='png', dpi=300)

#store odd data 
save(df_all_sex_odd_base, file = "data/issues/odd_data/df_all_sex_odd_base.RData")
...
df_all_coord_odd_base <- df_all_length_base %>% 
...
df_all_emus_no_coord_base <- df_all_length_base %>%
  semi_join(df_all_coord_odd_base, by = "ser_emu_nameshort") %>%
      mutate(coordinates_missing = is.na(ser_x) | is.na(ser_y),
         ser_y_out_of_range = ser_y > 70 | ser_y < 20,
         ser_x_out_of_range = ser_x > 53 | ser_x < -30) %>%
    mutate(issue = apply(select(., coordinates_missing:ser_x_out_of_range), 1, function(x) {
...
create_summary_coordinates <- function(lfs) {
  if(missing(lfs)) {
    filtered_coord <- df_all_emus_no_coord_base
  }
  else if(is.na(lfs)) {
    filtered_coord <- df_all_emus_no_coord_base %>% filter(is.na(lfs_code))
  } else {
    filtered_coord <- df_all_emus_no_coord_base %>% filter(lfs_code == lfs)
  }
  filtered_coord %>%
...
#apply function for each life stage
summary_all_emus_no_coord_base <- create_summary_coordinates()
summary_Y_emus_no_coord_base <- create_summary_coordinates("Y")
summary_S_emus_no_coord_base <- create_summary_coordinates("S")
summary_YS_emus_no_coord_base <- create_summary_coordinates("YS")
summary_NA_emus_no_coord_base <- create_summary_coordinates(NA)

#print tables
flextable(summary_all_emus_no_coord_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("TOTAL")

flextable(summary_Y_emus_no_coord_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = Y")

flextable(summary_S_emus_no_coord_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = S")

flextable(summary_YS_emus_no_coord_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = YS")

flextable(summary_NA_emus_no_coord_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE)))) %>%
  set_caption("lfs_code = NA")

#plot data with no coordinates
plot_all_coordinates_odd_base <- ggplot(df_all_emus_no_coord_base) +
...
#print plot
plot_all_coordinates_odd_base

#save csv of summaries                    
write.table(summary_Y_emus_no_coord_base, file = "data/issues/tables/summary_Y_emus_no_coord_base.csv", row.names = FALSE, sep = ";")
write.table(summary_S_emus_no_coord_base, file = "data/issues/tables/summary_S_emus_no_coord_base.csv", row.names = FALSE, sep = ";")
write.table(summary_YS_emus_no_coord_base, file = "data/issues/tables/summary_YS_emus_no_coord_base.csv", row.names = FALSE, sep = ";")
write.table(summary_NA_emus_no_coord_base, file = "data/issues/tables/summary_NA_emus_no_coord_base.csv", row.names = FALSE, sep = ";")

# save plots
ggsave("plot_all_coordinates_odd_base.png", plot_all_coordinates_odd_base, path = "data/issues/plots/", device = "png", dpi = 300, width = 16, height = 10, units = "in")

# store odd data
save(df_all_coord_odd_base, file = "data/issues/odd_data/df_all_coord_odd_base.RData")
...
df_all_obvious_odd_base <- df_all_length_base %>%
...
# remove odd eels from data
df_all_clean_base <- anti_join(df_all_length_base, df_all_obvious_odd_base, by = "mei_fi_id")

# create a summary table with removals
summary_all_obvious_odd_base <- df_all_obvious_odd_base %>% 
            group_by(lfs_code) %>% 
            summarize(Total = length(mei_fi_id),
...
#print summary table of removals
flextable(summary_all_obvious_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE))))

#create	summary of eels per life stage remaining in the dataset
summary_n_lfs_code_base <- df_all_clean_base %>%
  group_by(lfs_code) %>% 
  summarize(no_individuals = n()) %>% 
              bind_rows(summarize(., lfs_code = "Sum", across(where(is.numeric), sum)))

#print summary table of eels remaining in the dataset
flextable(summary_n_lfs_code_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE))))


#store tables as csv
write.table(summary_all_obvious_odd_base, file = "data/issues/tables/summary_all_obvious_odd_base.csv", row.names = FALSE, sep = ";")
write.table(summary_n_lfs_code_base, file = "data/issues/tables/summary_n_lfs_code_base.csv", row.names = FALSE, sep = ";")

# store odd data
save(df_all_obvious_odd_base, file = "data/issues/odd_data/df_all_obvious_odd_base.RData")
...
df_G_base <- df_all_clean_base %>% filter(lfs_code == "G")

#find eels with unrealistic values...
df_G_obvious_odd_base <- df_G_base %>% 
...
df_G_clean_base <- anti_join(df_G_base, df_G_obvious_odd_base, by = "mei_fi_id")

#fit weight length relationships for glass eel (quantile regression, tau .5)
model_G_wl_base <- rq(log(weightg)~log(lengthmm), tau = 0.5, data = df_G_clean_base)

# compute the residual thresholds 
residual_thresholds_G_base <- quantile(log(df_G_clean_base$weightg) - predict(model_G_wl_base, newdata = df_G_clean_base), 
                                c(0.0005, 0.9995), na.rm = TRUE)

# Apply residual thresholds to identify odd data
df_G_clean_base <- df_G_clean_base %>% 
  mutate(prediction = predict(model_G_wl_base, newdata = df_G_clean_base),
         residual = log(weightg) - prediction) %>%  # Use signed residuals
  mutate(prediction_lower = prediction + residual_thresholds_G_base[1],  # Lower bound
         prediction_upper = prediction + residual_thresholds_G_base[2],  # Upper bound
         weight_length_off = residual < residual_thresholds_G_base[1] | residual > residual_thresholds_G_base[2])

#extract data where weight length relationship is below/above threshold
df_G_wl_odd_base <- df_G_clean_base %>% 
  filter(weight_length_off == TRUE) %>%
  mutate(issue = "weight_length_off") %>% 
  select(-prediction, -residual, -prediction_upper, -prediction_lower) 

#create labels for plot
df_G_plot_base <-  df_G_clean_base %>% 
  mutate(issue = case_when(weight_length_off == TRUE ~ "weight_length_off", 
                           TRUE ~ "no_issue"))

#create summary table with removals
summary_G_wl_odd_base <- df_G_wl_odd_base %>% 
                    bind_rows(df_G_obvious_odd_base) %>% 
                    group_by(ser_cou_code) %>% 
...
flextable(summary_G_wl_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE))))


#plot faulty data
plot_G_wl_log_base <- ggplot(data = df_G_plot_base, aes(x = log(lengthmm), y = log(weightg))) +
  geom_ribbon(aes(ymin = prediction_lower, ymax = prediction_upper), fill = "gray70", alpha = 0.3) +
...
plot_G_wl_base <- ggplot(data = df_G_plot_base, aes(x = lengthmm, y = weightg)) +
  geom_ribbon(aes(ymin = exp(prediction_lower), ymax = exp(prediction_upper)), fill = "gray70", alpha = 0.3) +
  geom_point(aes(color = issue), shape = 16, alpha = 0.5, size = 1) +
  scale_color_manual(values = c("no_issue" = "#00204D", "weight_length_off" = "#FF9F00")) +  
  geom_line(aes(x = lengthmm, y = exp(prediction)), color = "black") +
  theme_bw()
...
#store tables as csv
write.table(summary_G_wl_odd_base, file = "data/issues/tables/summary_G_wl_odd_base.csv", row.names = FALSE, sep = ";")

# save plots
ggsave("plot_G_wl_log_base.png", plot_G_wl_log_base, path = "data/issues/plots/", device='png', dpi=300)
ggsave("plot_G_wl_base.png", plot_G_wl_base, path = "data/issues/plots/", device='png', dpi=300)
  
# store data
save(df_G_obvious_odd_base, file = "data/issues/odd_data/df_G_obvious_odd_base.RData")
save(df_G_wl_odd_base, file = "data/issues/odd_data/df_G_wl_odd_base.RData")

# store model
save(model_G_wl_base, residual_thresholds_G_base, file = "data/models/model_G_wl_base.RData")
...
#filter data for glass eel only
df_Y_base <- df_all_clean_base %>% filter(lfs_code == "Y")

#find eels with unrealistic values...
df_Y_obvious_odd_base <- df_Y_base %>% 
...
#...and create a clean df without them
df_Y_clean_base <- anti_join(df_Y_base, df_Y_obvious_odd_base, by = "mei_fi_id")

#fit weight length relationships for glass eel (quantile regression, tau .5)
model_Y_wl_base <- rq(log(weightg)~log(lengthmm), tau = 0.5, data = df_Y_clean_base)

# compute the residual thresholds 
residual_thresholds_Y_base <- quantile(log(df_Y_clean_base$weightg) - predict(model_Y_wl_base, newdata = df_Y_clean_base), 
                                c(0.0005, 0.9995), na.rm = TRUE)

# Apply residual thresholds to identify odd data
df_Y_clean_base <- df_Y_clean_base %>% 
  mutate(prediction = predict(model_Y_wl_base, newdata = df_Y_clean_base),
         residual = log(weightg) - prediction) %>%
  mutate(prediction_lower = prediction + residual_thresholds_Y_base[1],  # Lower bound
         prediction_upper = prediction + residual_thresholds_Y_base[2],  # Upper bound
         weight_length_off = residual < residual_thresholds_Y_base[1] | residual > residual_thresholds_Y_base[2])

#extract data where weight length relationship is below/above threshold
df_Y_wl_odd_base <- df_Y_clean_base %>% 
  filter(weight_length_off == TRUE) %>%
  mutate(issue = "weight_length_off") %>% 
  select(-prediction, -residual, -prediction_upper, -prediction_lower) 

#create labels for plot
df_Y_plot_base <-  df_Y_clean_base %>% 
  mutate(issue = case_when(weight_length_off == TRUE ~ "weight_length_off", 
                           TRUE ~ "no_issue"))

#create summary table of odd data
summary_Y_wl_odd_base <- df_Y_wl_odd_base %>%
            bind_rows(df_Y_obvious_odd_base) %>% 
            group_by(ser_cou_code) %>% 
            summarize(unrealistic_lenght = sum(unrealistic_length, na.rm = TRUE),
                      unrealistic_weight = sum(unrealistic_weight, na.rm = TRUE),
                      weight_length_off = sum(weight_length_off, na.rm = TRUE),
...
flextable(summary_Y_wl_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE))))

#plot faulty data
plot_Y_wl_log_base <- ggplot(data = df_Y_plot_base, aes(x = log(lengthmm), y = log(weightg))) +
  geom_ribbon(aes(ymin = prediction_lower, ymax = prediction_upper), fill = "gray70", alpha = 0.3) +
  geom_point(aes(color = issue), shape = 16, alpha = 0.5, size = 1) +
  scale_color_manual(values = c("no_issue" = "#00204D", "weight_length_off" = "#FF9F00")) +
  geom_line(aes(x = log(lengthmm), y = prediction), color = "black") +
...
plot_Y_wl_base <- ggplot(data = df_Y_plot_base, aes(x = lengthmm, y = weightg)) +
  geom_ribbon(aes(ymin = exp(prediction_lower), ymax = exp(prediction_upper)), fill = "gray70", alpha = 0.3) +
  geom_point(aes(color = issue), shape = 16, alpha = 0.5, size = 1) +
  scale_color_manual(values = c("no_issue" = "#00204D", "weight_length_off" = "#FF9F00")) +
  geom_line(aes(x = lengthmm, y = exp(prediction)), color = "black") +
  scale_x_continuous(limits = c(min(df_Y_plot_base$lengthmm), max(df_Y_plot_base$lengthmm[which(!is.na(df_Y_plot_base$weightg))]))) +
  theme_bw()
...
#store tables as csv
write.table(summary_Y_wl_odd_base, file = "data/issues/tables/summary_Y_wl_odd_base.csv", row.names = FALSE, sep = ";")

#store plots
ggsave("plot_Y_wl_log_base.png", plot_Y_wl_log_base, path = "data/issues/plots/", device='png', dpi=300)
ggsave("plot_Y_wl_base.png", plot_Y_wl_base, path = "data/issues/plots/", device='png', dpi=300)
  
#store data
save(df_Y_obvious_odd_base, file = "data/issues/odd_data/df_Y_obvious_odd_base.RData")
save(df_Y_wl_odd_base, file = "data/issues/odd_data/df_Y_wl_odd_base.RData")

#store model
save(model_Y_wl_base, residual_thresholds_Y_base, file = "data/models/model_Y_wl_base.RData")
...
country_codes_base <- unique(df_Y_plot_base$ser_cou_code)

# Function to create and save plot
create_plot <- function(country_code) {
  plot <- ggplot(data = df_Y_plot_base %>% filter(ser_cou_code == country_code), 
  ...
  # Save plot as PNG
  filename <- paste0("plot_Y_wl_log_ominous_base_", country_code, ".png")
  ggsave(filename, plot, path = "data/issues/plots/patterns",  width = 8, height = 6, dpi = 300)
  
}

# Generate and save all plots
plots_base <- map(country_codes_base, create_plot)

# Display only the FR plot in R Markdown
ggplot(data = df_Y_plot_base %>% filter(ser_cou_code == "FR"), 
                 aes(x = log(lengthmm), y = log(weightg))) +
    geom_point(shape = 16, alpha = 0.4, size = 1) +
    geom_line(aes(x = log(lengthmm), y = prediction), color = "black") +
    theme_bw() +
...
#filter data for glass eel only
df_S_base <- df_all_clean_base %>% filter(lfs_code == "S")

#find eels with unrealistic values...
df_S_obvious_odd_base <- df_S_base %>% 
  mutate(unrealistic_length = lengthmm < 150,
...
#...and create a clean df without them
df_S_clean_base <- anti_join(df_S_base, df_S_obvious_odd_base, by = "mei_fi_id")

#fit weight length relationships for glass eel (quantile regression, tau .5)
model_S_wl_base <- rq(log(weightg)~log(lengthmm), tau = 0.5, data = df_S_clean_base)

# compute the residual thresholds 
residual_thresholds_S_base <- quantile(log(df_S_clean_base$weightg) - predict(model_S_wl_base, newdata = df_S_clean_base), 
                                c(0.0005, 0.9995), na.rm = TRUE)

# Apply residual thresholds to identify odd data
df_S_clean_base <- df_S_clean_base %>% 
  mutate(prediction = predict(model_S_wl_base, newdata = df_S_clean_base),
         residual = log(weightg) - prediction) %>%
  mutate(prediction_lower = prediction + residual_thresholds_S_base[1],  # Lower bound
         prediction_upper = prediction + residual_thresholds_S_base[2],  # Upper bound
         weight_length_off = residual < residual_thresholds_S_base[1] | residual > residual_thresholds_S_base[2])

#extract data where weight length relationship is below/above thresholds
df_S_wl_odd_base <- df_S_clean_base %>% 
  filter(weight_length_off == TRUE) %>%
  mutate(issue = "weight_length_off") %>% 
  select(-prediction, -residual, -prediction_upper, -prediction_lower) 

#create labels for plot
df_S_plot_base <-  df_S_clean_base %>% 
  mutate(issue = case_when(weight_length_off == TRUE ~ "weight_length_off", 
                           TRUE ~ "no_issue"))

#create summary table of odd data
summary_S_wl_odd_base <- df_S_wl_odd_base %>%
            bind_rows(df_S_obvious_odd_base) %>% 
            group_by(ser_cou_code) %>% 
            summarize(unrealistic_lenght = sum(unrealistic_length, na.rm = TRUE),
                      unrealistic_weight = sum(unrealistic_weight, na.rm = TRUE),
                      weight_length_off = sum(weight_length_off, na.rm = TRUE),
...
#print summary table of odd data
flextable(summary_S_wl_odd_base %>%
            mutate(across(everything(), ~format(.x, big.mark = " ", scientific = FALSE))))

#plot faulty data
plot_S_wl_log_base <- ggplot(data = df_S_plot_base, aes(x = log(lengthmm), y = log(weightg))) +
  geom_ribbon(aes(ymin = prediction_lower, ymax = prediction_upper), fill = "gray70", alpha = 0.3) +
  geom_point(aes(color = issue), shape = 16, alpha = 0.5, size = 1) +
  scale_color_manual(values = c("no_issue" = "#00204D", "weight_length_off" = "#FF9F00")) +
  geom_line(aes(x = log(lengthmm), y = prediction), color = "black") +
...
plot_S_wl_base <- ggplot(data = df_S_plot_base, aes(x = lengthmm, y = weightg)) +
  geom_ribbon(aes(ymin = exp(prediction_lower), ymax = exp(prediction_upper)), fill = "gray70", alpha = 0.3) +
  geom_point(aes(color = issue), shape = 16, alpha = 0.5, size = 1) +
  scale_color_manual(values = c("no_issue" = "#00204D", "weight_length_off" = "#FF9F00")) +
  geom_line(aes(x = lengthmm, y = exp(prediction)), color = "black") +
...
#store tables as csv
write.table(summary_S_wl_odd_base, file = "data/issues/tables/summary_S_wl_odd_base.csv", row.names = FALSE, sep = ";")

#store plots
ggsave("plot_S_wl_log_base.png", plot_S_wl_log_base, path = "data/issues/plots/", device='png', dpi=300)
ggsave("plot_S_wl_base.png", plot_S_wl_base, path = "data/issues/plots/", device='png', dpi=300)
  
#store data
save(df_S_obvious_odd_base, file = "data/issues/odd_data/df_S_obvious_odd_base.RData")
save(df_S_wl_odd_base, file = "data/issues/odd_data/df_S_wl_odd_base.RData")

#store model
save(model_S_wl_base, residual_thresholds_S_base, file = "data/models/model_S_wl_base.RData")
